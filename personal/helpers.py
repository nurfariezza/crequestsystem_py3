...
def autov_filter(req):
    #start = datetime(2018, 7, 10)
    start = datetime.today()
   
    end_month = datetime.today() - timedelta(days=60)# datetime(2018, 5, 10)
    print(start)
    b = models.ReqForm.objects.filter(completeddate__gte=end_month).filter(completeddate__lte=start).filter(reqstatus=6).filter(verified=0)
    #b = b.exclude(reqstatus=9)
