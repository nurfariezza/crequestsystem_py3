...
def loginview(req, error=None):
    ...
    url = q.format(settings.CLIENT_ID, settings.REDIRECT_URI, nexturl)
    ctx = {
        'next': nexturl,
        'url': url,
    ...
    if 'email' in m:
        ...
        try:
            ...
            if user.email is not None:
                ...
                nexturl = 'home' if state in [None, 'None'] else state
...
def pushnotify(req, pid):
    try:
        ...
        if o.reqstatus == 4:
                ...
                sendemail(render_to_string('personal/pushnoti.html', dic), head, email)
